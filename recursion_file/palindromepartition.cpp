  ...
  void func(int index, string &s, vector < string > & path,
    ...
    for (int i = index; i < s.size(); ++i) {
      if (isPalindrome(s, index, i)) {
        string top = s.substr(index, i - index + 1);
        path.push_back(top);
        cout<<top<<" "<<endl;
        func(i + 1, s, path, res);
        path.pop_back();
...
if the substring generated is a palindrome. O(k) is for inserting the palindromes in another data structure, where k  is the average length of the palindrome list.
